...
func (d *UserRepositoryDecorator) FindByUsername(username string) (*entities.User, error) {
	...
	if found {
		user, err := d.getUserModelFromCachedData(data)
		if err != nil {
			return nil, err
		}
		return user, nil
	...
	cachingData, err := d.marshalUser(user)
	if err != nil {
		return user, nil
	}
...
func (d *UserRepositoryDecorator) getUserModelFromCachedData(cachedData interface{}) (*entities.User, error) {
...
func (d *UserRepositoryDecorator) marshalUser(user *entities.User) (interface{}, error) {
